/**
 * Configuration file management for tidy (Raycast version)
 *
...
import type { TidyConfig, ModelSelection } from "./types";
 ...
 * (Profile support disabled for Raycast to avoid dependency complexity)
 */
export function resolveConfigWithProfile(
  profileName?: string,
  basePath: string = process.cwd(),
): TidyConfig {
  return resolveConfig(basePath);
}
...
export function getRulesPromptWithProfile(
  ...
  return getRulesPrompt(basePath);
}
