...
import { moveFile, fileExists, formatFileSize } from "../utils/files.ts";
import { getFileIcon, getCategoryIcon, getStatusIndicator } from "../utils/icons.ts";
import {
  resolveConfig,
  expandPath,
  parseModelString,
...
function displayProposal(proposal: FileMoveProposal, index: number): void {
  ...
  const confidenceColor =
    confidence >= 80 ? color.green : confidence >= 50 ? color.yellow : color.red;
...
async function executeProposals(
  ...
  for (let i = 0; i < proposals.length; i++) {
    ...
    s.start(
      `Moving ${i + 1}/${proposals.length}: ${prop.file.name}`
    );
...
async function viewProposalDetails(
  ...
  p.log.message(
    `  Confidence: ${Math.round(prop.category.confidence * 100)}%`
  );
...
export async function organizeCommand(options: OrganizeOptions): Promise<void> {
  p.intro(color.bgGreen(color.black(" tidy ")));
      ...
      switch (action) {
        ...
        case "regenerate": {
          const newInstructions = await p.text({
            ...
            placeholder:
              "e.g., Keep all PDFs together, sort images by date",
          });
